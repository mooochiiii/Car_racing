...
document.addEventListener("keydown", (e) => {
    ...
    if (is_start == true && is_finished == false) {
        if (keysPressed['d']) {
            if (yellow_pos < 600) {
                yellow_car.style.left = yellow_pos + 40 + 'px';
                yellow_pos += 40
                yellow_fuel.style.width = 120 - (120 / 600 * yellow_pos) + 'px'
            } else {
        ...
        if (keysPressed['ArrowRight']) {
            if (red_pos < 600) {
                red_car.style.left = red_pos + 40 + 'px';
                red_pos += 40
                red_fuel.style.width = 120 - (120 / 600 * red_pos) + 'px'
            } else {
...
function resetGame() {
    ...
    for (var i = 0, len = traffic_light.length; i < len; i++) {
        traffic_light[i].style["background-color"] = 'black';
    }
